...
- OAuth 登录模式点击收藏按钮直接打开完整编辑 panel；状态、评分、评价、标签和隐私在
  同一个主题化、本地化 panel 内编辑，并保留取消收藏入口。未登录模式继续使用紧凑状态选择器。
- panel 展示条目标签，点击即可加入我的标签；标签输入支持回车 / 逗号确认、去重和校验。
- 收藏新增、更新和读取 / 写入失败提示均区分 Bangumi 云端语义并完成中英文本地化；成功
  写入后继续回读服务端规范化结果。
- 增加详情页 panel 的中英文 widget 测试；ARB、repository、controller 和详情页测试均通过。

剩余工作仅为真实账号的增改清空与反代路由人工验收，不影响代码接口和自动化测试完成度。
