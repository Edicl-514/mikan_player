...
pub async fn fetch_archive_list() -> anyhow::Result<Vec<ArchiveQuarter>> {
    ...
    for element in document.select(&selector) {
        if element.value().name() == "h3" {
            current_year = element.text().collect::<String>().replace("年", "").trim().to_string();
...
pub async fn fetch_schedule_basic(year_quarter: String) -> anyhow::Result<Vec<AnimeInfo>> {
...

    {
        ...
        for root in document.select(&root_selector) {
            let title = root.select(&title_selector)
                .next()
            ...
            let sub_title = root.select(&sub_title_selector)
                .next()
                .map(|e| e.text().collect::<String>().trim().to_string());

            let time_str = root.select(&time_selector)
                .next()
                .map(|e| e.text().collect::<String>().trim().to_string());
...
pub async fn fetch_extra_subjects(year_quarter: String, existing_ids: Vec<String>) -> anyhow::Result<Vec<AnimeInfo>> {
    let client = reqwest::Client::builder()
        .user_agent("MikanPlayer/1.0 (https://github.com/your-repo/mikan_player)")
        .build()?;
...
async fn fetch_extra_bangumi_subjects(
    ...
    for i in 0..num_pages {
        ...
        tasks.push(tokio::spawn(async move {
            let resp = client_c.post("https://api.bgm.tv/v0/search/subjects")
        ...
        if let Ok(Some(json)) = task.await {
            if let Some(data) = json["data"].as_array() {
                for item in data {
                    ...
                    let title = if !name_cn.is_empty() { name_cn.to_string() } else { name.to_string() };
                    ...
                    let rank = item["rank"].as_i64()
                        .or_else(|| item["rating"]["rank"].as_i64())
                        .map(|r| r as i32);

                    let anime = AnimeInfo {
                        title: title.clone(),
                        sub_title: if name_cn.is_empty() { None } else { Some(name.to_string()) }, 
